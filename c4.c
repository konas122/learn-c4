...
// 因为作者没有实现结构体,所以 [id] 指向的空间被分割为 Idsz 大小的块 (模拟结构体)
// 当 id 指向块首时, id[0] == id[Tk] 访问的就是Tk成员的数据 (一般是指针)
// Name 指向的是这个 identifier 的 Name
// Type 为数据类型 (比如返回值类型), 如 CHAR, INT, INT+PTR
// Class 为类型, 如 Num (常量数值), Fun (函数), Sys (系统调用), Glo 全局变量, Loc 局部变量
...
void next()
{
    ...
    while (tk = *p) {
        ...
        if (tk == '\n') {
            ...
            pp = p - 1;     // 因为有 ++p, pp 回退一个字符, pp 指向 [这个符号] 的首字母
...
// 表达式分析
// lev 表示运算符, 因为各个运算符 token 是按照优先级生序排列的, 所以 lev 大表示优先级高
void expr(int lev) {
    int t, *d;
    ...
    if (!tk) { printf("%d: unexpected eof in expression\n", line); exit(-1); }
        ...
        *++e = (ty == CHAR) ? sizeof(char) : sizeof(int);   // 除了char是一字节,int和多级指针都是int大小
        ...
        if (tk == '(') {    // 函数
            ...
            if (d[Class] == Sys) *++e = d[Val];     // 系统调用, 如 malloc, memset, d[val] 为 opcode
        ...
        else if (d[Class] == Num) { *++e = IMM; *++e = d[Val]; ty = INT; }  // d[Class] == Num, 处理枚举(只有枚举是 Class==Num)
        ...
        *++e = (ty == CHAR) ? LC : LI;
        ...
        if (*e == LC || *e == LI) --e;  // 根据上面的代码, token 为变量时都是先取地址再 LI/LC, 所以 --e 就变成了取地址到 a
        else { printf("%d: bad address-of\n", line); exit(-1); }

        ty = ty + PTR;
    }

    else if (tk == '!') { next(); expr(Inc); *++e = PSH; *++e = IMM; *++e = 0; *++e = EQ; ty = INT; }   // !x 相当于 x==0
    else if (tk == '~') { next(); expr(Inc); *++e = PSH; *++e = IMM; *++e = -1; *++e = XOR; ty = INT; } // ~x 相当于 x^-1
    ...
    // x--, x++ 在后面处理
        ...
        // 处理 ++x, --x
        if (*e == LC) { *e = PSH; *++e = LC; }  // 地址压栈(下面 SC/SI 用到), 再取数
    ...
    // 爬山法
    // tk 为 ASCII 码的都不会超过 Num=128
    while (tk >= lev) { // "precedence climbing" or "Top Down Operator Precedence" method
        t = ty;         // ty 在递归过程中可能会改变, 所以备份当前处理的表达式类型

        if (tk == Assign) { // 赋值
            next();

            if (*e == LC || *e == LI) *e = PSH;     // 左边被 tk=Id 中变量部分处理过了, 将地址压栈
            else { printf("%d: bad lvalue in assignment\n", line); exit(-1); }

            expr(Assign);
            *++e = ((ty = t) == CHAR) ? SC : SI;    // 取得右值 expr 的值, 作为 a=expr 的结果
        }

        else if (tk == Cond) {  // x?a:b 和 if 类似, 除了不能没有 else
            next();
            *++e = BZ;
            d = ++e;
            expr(Assign);
        ...
        else if (tk == Lor) { next(); *++e = BNZ; d = ++e; expr(Lan); *d = (int)(e + 1); ty = INT; }    // 短路, 逻辑 Or 运算符左边为 true 则表达式为 true, 不用计算运算符右侧的值
        else if (tk == Lan) { next(); *++e = BZ;  d = ++e; expr(Or);  *d = (int)(e + 1); ty = INT; }    // 短路, 逻辑 And, 同上

        else if (tk == Or)  { next(); *++e = PSH; expr(Xor); *++e = OR;  ty = INT; }    // 将当前值 Push, 计算运算符右边值, 再与当前值 (在栈中) 做运算;
        else if (tk == Xor) { next(); *++e = PSH; expr(And); *++e = XOR; ty = INT; }    // expr 中 lev 指明递归函数中最结合性不得低于哪一个运算符
        ...
        // 数组下标
            ...
            *++e = PSH;
            ...
            if (t > PTR) { *++e = PSH; *++e = IMM; *++e = sizeof(int); *++e = MUL;  }   // t==PTR 时是 Char, Char = 0
